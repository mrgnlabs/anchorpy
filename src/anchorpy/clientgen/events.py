...
def gen_events(idl: Idl, root: Path) -> None:
    events = idl.events
    if events is None or not events:
        return
    events_dir = root / "events"
    events_dir.mkdir(exist_ok=True)
    gen_index_file(idl, events_dir)
    events_dict = gen_events_code(idl, events_dir)
    for path, code in events_dict.items():
        formatted = format_str(code, mode=FileMode())
        fixed = fix_code(formatted, remove_all_unused_imports=True)
        path.write_text(fixed)


def gen_index_file(idl: Idl, events_dir: Path) -> None:
    code = gen_index_code(idl)
    formatted = format_str(code, mode=FileMode())
    (events_dir / "__init__.py").write_text(formatted)


def gen_index_code(idl: Idl) -> str:
    imports: list[FromImport] = []
    for evt in idl.events:
        evt_name = _sanitize(evt.name)
        members = [
            evt_name,
            _json_interface_name(evt_name),
        ]
        module_name = _sanitize(snake(evt.name))
        imports.append(FromImport(f".{module_name}", members))
    return str(Collection(imports))


def gen_events_code(idl: Idl, events_dir: Path) -> dict[Path, str]:
    res = {}
    for evt in idl.events:
        filename = f"{_sanitize(snake(evt.name))}.py"
        path = events_dir / filename
        code = gen_event_code(evt, idl)
        res[path] = code
    return res


def gen_event_code(event: IdlTypeDefinition, idl: Idl) -> str:
    base_imports = [
        Import("typing"),
        ...
        FromImport("anchorpy.coder.event", ["EVENT_DISCRIMINATOR_SIZE"]),
        FromImport("anchorpy.error", ["EventInvalidDiscriminator"]),
        FromImport(
    ...
    discriminator_assignment = Assign(
        "discriminator: typing.ClassVar", _event_discriminator(name)
    )
    ...
    event_invalid_raise = Raise(
        'EventInvalidDiscriminator("The discriminator for this event is invalid")'
    )
    decode_method = ClassMethod(
        ...
        Suite(
            [
                If(
                    "data[:EVENT_DISCRIMINATOR_SIZE] != cls.discriminator",
                    event_invalid_raise,
                ),
                Assign(
                    "dec", f"{name}.layout.parse(data[EVENT_DISCRIMINATOR_SIZE:])"
                ),
